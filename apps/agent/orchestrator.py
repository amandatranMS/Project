...
ORCHESTRATOR_INSTRUCTIONS = (
    ...
    "tool returned it for that scope. This restriction does not prevent explicitly labeled "
    "assumptions in a requested new-record draft. Do not mix dashboard metrics into a single-opportunity "
    ...
    "opportunity, treat that request as permission to draft now: delegate to retrieve available "
    "context and return the complete editable draft in the same response. Never ask whether to "
    "proceed with drafting or ask for individual fields before the first draft. Infer reasonably "
    "inferable values from that context and controlled choices instead "
