...
import { sendChatStream, type ChatTurn } from '../api/client';
...
const ENGINE_LABEL = 'Foundry hosted agent';
...
function newConversation(): Conversation {
  return { id: genId(), title: 'New chat', messages: [], updatedAt: Date.now() };
}

/**
 * Does the assistant's closing line ask the user to sign off before it submits?
 *
 * Chat runs on the hosted Foundry agent, whose instructions live in Foundry — not in
 * this repo — so it words this ask freely ("Shall I submit this?",
  ...
  if (owner === 'local') {
    ...
    try {
      ...
      if (Array.isArray(msgs) && msgs.length) {
        ...
        return [{ id: genId(), title: deriveTitle(msgs), messages: msgs, updatedAt: Date.now() }];
      }
    } catch {
      /* ignore */
    }
...
export default function ChatWidget() {
  ...
  const runTurn = useCallback(async (convoId: string, transcript: StoredTurn[]) => {
    ...
    try {
      ...
      await sendChatStream(transcript, appendToAssistant, controller.signal);
    } catch (err) {
