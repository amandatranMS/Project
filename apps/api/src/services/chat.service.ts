...
import type { ChatMessage, TokenSink } from './chat/types.js';
...
export const chatService = {
  /**
   * Runs one assistant turn against the deployed Foundry hosted agent.
   *
   * If `onToken` is supplied the answer is forwarded as it arrives; the agent's
   * own tool calls come back as separate HTTP requests, so they aren't streamed.
   *
   ...
   * governed action.
   */
  async send(messages: ChatMessage[], user?: AuthUser, onToken?: TokenSink) {
